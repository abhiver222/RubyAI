        ...
        {isPopulated(filteredMessages) ? (
          filteredMessages.map((message, index) => (
            <Box key={index} sx={{ marginBottom: "20px" }}>
              <Card
                variant="outlined"
                sx={{ backgroundColor: "#4d4d4d", p: 2 }}
              >
                <Typography variant="h6">{`${getCardHeading(
                  message.generation_data.medium,
                  message.generation_data.name
                )}`}</Typography>
                <TextareaAutosize
                  minRows={4}
                ...
                {isPopulated(message.feedback) && (
                  <Box sx={{ mt: 1 }}>
                    <label style={{ color: "#fff", marginBottom: 0 }}>
                      Feedback:
                    </label>
                    <TextareaAutosize
                      minRows={1}
                ...
                <Box sx={{ marginTop: "8px" }}>
                  <Accordion>
                    ...
                    <AccordionDetails>
                      <ul>
                        {Object.entries(message.generation_data).map(
                          ([key, value]) => (
